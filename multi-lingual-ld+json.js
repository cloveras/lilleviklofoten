add_action('wp_head', function () {
    ...
    // Handle specific locales like 'zh_TW' or 'en_GB'
    ...
    // Multilingual descriptions (unchanged)
    $descriptions = [
    ...
    // Fully expanded amenity mapping for all languages and all amenities
    $amenity_mapping = [
        // WiFi
    ...
    foreach ($localizedAmenities as $name) {
        $feature = [
            "@type" => "PropertyValue",
            "name" => $name,
            "value" => true // Indicates the amenity is available
        ];
        // Assign the standardized propertyID if it exists in our map
    ...
    echo '<script type="application/ld+json">' .
        wp_json_encode([$organization, $lodging], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT) .
        '</script>';
});
